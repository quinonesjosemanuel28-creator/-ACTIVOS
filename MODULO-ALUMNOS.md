...
### Cerradas en el ticket 10 (agosto 2026) — notas y bitácora

La decisión que ordena el ticket: **las consultas se resuelven por WhatsApp; la plataforma es el registro, no el canal** (el canal completo con derivación y rol de referente quedó descartado hasta que exista volumen real). TICKET-10.md rev2 es la especificación.

- **El ciclo de vida de la nota es DERIVADO** (`nota_resoluciones`, append-only): sin resolución la nota está abierta — cero migración sobre `checkins`, que sigue siendo un stream puro. La resolución más nueva gana: corregir una devolución = re-resolver. Las audiencias las custodia el schema: responder EXIGE devolución (la ve el alumno en su link) y archivar la PROHÍBE.
- **La etiqueta de área** (legal/contable/marketing/ventas/estructura/otra, enum solo en Zod) al resolver o archivar: con volumen, dice qué área concentra las trabas de la cartera — qué clase le falta a la Academy.
- **El panel muestra el contador de notas abiertas por fila**: sin notificaciones (descartadas), es la única forma de descubrir una nota nueva sin abrir cada ficha.
- **Al link baja solo la última nota propia por acción y su devolución si está resuelta** — una archivada no muestra nada nuevo; sin estados intermedios ni nada que parezca un ticket de soporte. WhatsApp con la nota citada (`mensajeNota`) para el flujo real: conversación afuera, devolución en dos líneas adentro.
