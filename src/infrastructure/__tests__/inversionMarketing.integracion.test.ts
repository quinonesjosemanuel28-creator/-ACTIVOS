...
function setup() {
  ...
  cierres.cierres.guardar({ idCierre: 'C1', fechaCierre: '2026-05-02', clienteNombre: 'X', programa: 'Empresario', ticketTotalUsd: 3000, closer: 'Ana', unidadNegocio: 'ACADEMY', estado: 'Activo' });
  cierres.pagos.guardar({ idPago: 'P1', idCierre: 'C1', fechaPago: '2026-05-03', montoUsd: 3000, tipoPago: 'Pago Completo', medioPago: 'Otro' });
  // Egresos de mayo: dos de marketing (suman 3500) + uno de otra categoría.
  uce.crearEgreso(egRepo, { fecha: '2026-05-04', categoria: 'Marketing y publicidad', concepto: 'IG', montoUsd: 2000 });
  uce.crearEgreso(egRepo, { fecha: '2026-05-10', categoria: 'Marketing y publicidad', concepto: 'Google', montoUsd: 1500 });
  uce.crearEgreso(egRepo, { fecha: '2026-05-06', categoria: 'Sueldos', concepto: 'equipo', montoUsd: 4000 });
  return { db, egRepo };
}

describe('Inversión Marketing del dashboard ↔ egresos de marketing', () => {
  it('inversionMarketing = suma de egresos categoría marketing del mes (3500)', () => {
    const { db, egRepo } = setup();
    const snap = uc.obtenerDashboardDelMes(crearRepositoriosDashboard(db), '2026-05').snapshot;

    const marketingEgresos = uce
      .listarEgresos(egRepo, { mes: '2026-05' })
  ...
  it('CAC/ROAS/MER se vuelven calculables al conectar la fuente', () => {
    const { db } = setup();
    const snap = uc.obtenerDashboardDelMes(crearRepositoriosDashboard(db), '2026-05').snapshot;
  ...
  it('utilidad y egresos totales siguen leyendo toda la fuente (no se rompen)', () => {
    const { db } = setup();
    const snap = uc.obtenerDashboardDelMes(crearRepositoriosDashboard(db), '2026-05').snapshot;
