...
export const NOTAS_NEGOCIO = `Notas importantes del modelo de datos (SQLite):
...
- pagos.fecha_pago = fecha del cobro, formato 'YYYY-MM-DD' (TEXT). NO existe una columna "mes" en pagos: para filtrar por mes usá strftime('%Y-%m', fecha_pago) o substr(fecha_pago,1,7).
...
export const EJEMPLOS_SQL = `Ejemplos de cómo se consultan cosas comunes (seguí estos patrones):

P: ¿Cuánto facturé en total?
SQL: SELECT SUM(monto_usd) AS facturacion_usd FROM pagos

P: ¿Cuánto facturé en mayo 2026?
SQL: SELECT SUM(monto_usd) AS facturacion_usd FROM pagos WHERE strftime('%Y-%m', fecha_pago) = '2026-05'
...
SQL: SELECT c.cliente_nombre, c.ticket_total_usd - COALESCE((SELECT SUM(p.monto_usd) FROM pagos p WHERE p.id_cierre = c.id_cierre),0) AS saldo_usd, c.fecha_primera_cuota FROM cierres c WHERE c.cantidad_cuotas IS NOT NULL AND c.inactivo = 0 AND c.fecha_primera_cuota IS NOT NULL AND date(c.fecha_primera_cuota) < date('now','-60 day') AND (c.ticket_total_usd - COALESCE((SELECT SUM(p.monto_usd) FROM pagos p WHERE p.id_cierre = c.id_cierre),0)) > 0`;

/** Arma el bloque de contexto completo para el prompt de generación de SQL. */
export function contextoSql(esquemaReal: string): string {
  return [
